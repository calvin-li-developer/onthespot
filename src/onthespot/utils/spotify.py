...
def sanitize_data(value, allow_path_separators=False, escape_quotes=False):
    ...
    if os.name == 'nt':
        ...
        value = os.path.join(
            drive_letter,
            tail.replace(':', '꞉') # replace ":" with "꞉(U+A789)"
        )
        value = value.rstrip('.')
    else:
...
def set_audio_tags(filename, metadata, track_id_str):
    ...
    for key in metadata.keys():
        ...
        if key == 'artists':
            ...
            tags['albumartist'] = conv_artist_format(value)
        elif key in ['name', 'track_title', 'tracktitle']:
...
def get_song_info(session, song_id):
    ...
    album_artists = []
    for data1 in info['tracks'][0]['artists']:
    ...
    contributing_artists.remove(album_artists[0])
    info = {
